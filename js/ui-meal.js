...
(function (global) {
    ...
    if (!items.length) {
      ...
      items.forEach(function (e) {
        ...
        h += '<div class="item" data-entry="' + A().esc(e.id) + '">' +
          '<div class="grow"><div class="item-name ellip">' + A().esc(e.name) + registered +
          (e.brand ? ' <span class="item-brand">' + A().esc(e.brand) + '</span>' : '') + '</div>' +
  ...
  function fromMyFood(m) {
    ...
    return {
      ...
      ref: { type: 'my', id: m.id }, note: m.brand || '', brand: m.brand || '',
      est: mergedEst, linked: !!m.linked
    };
  }
  function fromEntry(e) {
    ...
    return {
      ...
      note: e.brand || '', brand: e.brand || '',
      est: cleanEstimate(e.est, masterMerge.appliedKeys), seq: e.seq
    };
  }
  ...
  function openAmount(state, slot, pick, existingId, onPick) {
    ...
    fill.addEventListener('click', function () {
      ...
      var preset = {
        name: pick.name, brand: pick.brand || '', basis: isG ? '100g' : 'serving',
  ...
  function openManual(state, slot, preset, existingId, onPick) {
    ...
    var html = '' +
      ...
      '</div>' +
      /* 読み取りは iPhone 本体の文字認識にまかせる。長押しの「テキストをスキャン」なら
         カメラが画面下半分に開き、読んだ文字がこの欄に直接入る。アプリとカメラを行き来せずに済む。
         コピーして貼る道もあるが、行き来の分だけ遅いので逃げ道にとどめる */
      '<div class="card" id="ocrCard"><h3>栄養成分表示を読み取る</h3>' +
        '<div class="small" style="margin:0 2px 7px">' +
        '下の欄を<b>長押し</b>して、メニューの<b>「テキストをスキャン」</b>を押すとカメラが起動します。' +
        '読み取った文字は<b>そのまま下の各欄に入ります</b>。</div>' +
        '<textarea id="mOcr" class="ocr-input" rows="7" ' +
        'placeholder="ここを長押し →「テキストをスキャン」"></textarea>' +
        '<details class="nut-more" id="ocrTextBox">' +
          '<summary>「テキストをスキャン」が出ないとき</summary>' +
          '<div class="small muted" style="margin:0 2px 7px">' +
          '標準カメラか写真アプリで文字を選んで「コピー」し、下のボタンを押してください。' +
          '読み取りの精度は同じです。</div>' +
    ...
    if (paste) paste.addEventListener('click', function () {
      // 写真の文字をコピーしてきた場合の逃げ道。取れないときは手で貼れるように欄へ案内する
      var manual = function (msg) { ocr.focus(); A().toast(msg); };
      var nav = global.navigator;
      if (!nav || !nav.clipboard || !nav.clipboard.readText) {
        manual('読み取り欄を長押しして「ペースト」を選んでください');
        return;
      }
      nav.clipboard.readText().then(function (text) {
        if (!text || !text.trim()) {
          manual('コピーされた文字がありません。先に写真の文字を選んで「コピー」してください');
          return;
        }
        ocr.value = text;
        applyOcr(false);
    ...
    body.querySelector('#mSave').addEventListener('click', function () {
      ...
      estimateJob.catch(function () { return null; }).then(function (filled) {
        return S.MyFoods.byName(name).then(function (sameName) {
          // 欄が空のときは、すでに登録されているメーカー名を消さない
          var typedBrand = body.querySelector('#mBrand').value.trim();
          var keptBrand = typedBrand || preset.brand || (sameName && sameName.brand) || '';
          var rec = {
            id: preset.myFoodId || (sameName && sameName.id) || undefined,
            name: name, brand: keptBrand,
  ...
  function openBarcodeUnknown(state, slot, code) {
    var html = '<div class="card"><b>商品情報が見つかりませんでした</b>' +
      ...
      '読み取り欄が開きます。その欄を長押し →「テキストをスキャン」でカメラが起動し、' +
